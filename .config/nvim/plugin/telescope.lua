safe_require("telescope", function(telescope)
  ...
  map("<leader>ff", "<cmd>lua require('projects')[0].find_files()<cr>")
  ...
  map(
    "<leader>f.",
    "<cmd>lua require('telescope.builtin').find_files({cwd=string.format('%s/.config/nvim/', os.getenv('HOME')), follow=true, hidden=true, previewer=false})<cr>"
  )
  map(
    "<leader>fl",
